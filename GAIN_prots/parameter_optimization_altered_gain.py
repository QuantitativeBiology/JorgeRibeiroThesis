...
def suggest_custom(name, trial, nr_layers):
    ...
    if name == 'discr_size_layers':
        ...
        for i in range(1, nr_layers):
            discr_size_layers[i] = trial.suggest_float('discr_size_layers_{}'.format(i), 0.1, 1, step= 0.1)
        return discr_size_layers
    elif name == 'gen_size_layers':
        gen_size_layers = list(range(0, nr_layers))
        gen_size_layers[0] = 1
        for i in range(1, nr_layers):
            gen_size_layers[i] = trial.suggest_float('gen_size_layers_{}'.format(i), 0.1, 1, step=0.1)
        return gen_size_layers
    else:
        raise ValueError('Unknown parameter: {}'.format(name))


# GAIN function with number of hidden layers and size of hidden layers as variables
def gain_altered (data_x, gain_parameters, ori_data_x):
    ...
    discr_nr_layers = gain_parameters['discr_nr_layers']
    gen_nr_layers = gain_parameters['gen_nr_layers']
    discr_size_layers = gain_parameters['discr_size_layers']
    gen_size_layers = gain_parameters['gen_size_layers']
    ...
    #adapt size of hidden layers with the last value being h_dim
    discr_size_layers= [int(x * dim*2) for x in discr_size_layers]
    gen_size_layers= [int(x * dim*2) for x in gen_size_layers]
    discr_size_layers.append(h_dim)
    gen_size_layers.append(h_dim)
    ...
    # Discriminator variables with a variable number of hidden layers and size of hidden layers
    D_W = list(range(0, discr_nr_layers))
    D_b = list(range(0, discr_nr_layers))
    for n in range(1, discr_nr_layers):
        D_W[n] = tf.Variable(xavier_init([discr_size_layers[n], discr_size_layers[n+1]])) #last layer size has to be hdim
        D_b[n] = tf.Variable(tf.zeros(shape=[discr_size_layers[n+1]]))

    D_W_out = tf.Variable(xavier_init([h_dim, dim]))
    D_b_out = tf.Variable(tf.zeros(shape=[dim]))  # Multi-variate outputs

    theta_D = [D_W, D_b, D_W_out, D_b_out]

    # Generator variables with a variable number of hidden layers and size of hidden layers in each layer
    G_W = list(range(0, gen_nr_layers))
    G_b = list(range(0, gen_nr_layers))
    for n in range(1, gen_nr_layers):
        G_W[n] = tf.Variable(xavier_init([gen_size_layers[n], gen_size_layers[n+1]])) #last layer size has to be hdim
        G_b[n] = tf.Variable(tf.zeros(shape=[gen_size_layers[n+1]]))

    G_W_out = tf.Variable(xavier_init([h_dim, dim]))
    G_b_out = tf.Variable(tf.zeros(shape=[dim]))

    theta_G = [G_W, G_b, G_W_out, G_b_out]
    ...
    def generator(x, m):
        ...
        for n in range(0, gen_nr_layers):
            G_h[n] = tf.nn.relu(tf.matmul(inputs, G_W[n]) + G_b[n])
        # MinMax normalized output
        G_prob = tf.nn.sigmoid(tf.matmul(G_h[gen_nr_layers], G_W_out) + G_b_out)
        return G_prob
    ...
    def discriminator(x, h):
        ...
        for n in range(0, discr_nr_layers):
            D_h[n] = tf.nn.relu(tf.matmul(inputs, D_W[n]) + D_b[n])
        D_logit = tf.matmul(D_h[discr_nr_layers], D_W_out) + D_b_out
...
def objective(trial):
    ...
    gain_parameters = {
        ...
        'batch_size': 128,
        'hint_rate': trial.suggest_float('hint_rate', 0, 0.9, step=0.01),
        #'alpha': trial.suggest_float('alpha', 10, 10000, step= 1),
        'alpha': 100,
        'iterations': int(trial.suggest_float('iterations', 100, 10000, step=1)),
        'discr_nr_layers': trial.suggest_categorical('discr_nr_layers', [ 2, 3, 4, 5, 6, 7, 8, 9, 10]),
        'gen_nr_layers': trial.suggest_categorical('gen_nr_layers', [ 2, 3, 4, 5, 6, 7, 8, 9, 10]),
    }

    gain_parameters['discr_size_layers'] = suggest_custom('discr_size_layers', trial, gain_parameters['discr_nr_layers'])
    gain_parameters['gen_size_layers'] = suggest_custom('gen_size_layers', trial, gain_parameters['gen_nr_layers'])

    print('Parameters: {}'.format(gain_parameters))
...
if __name__ == '__main__':  
    ...
    #set folder name
    folder_name = '21_Oct_variable_layers_alpha_1000'
